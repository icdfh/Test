...
long_text = ''.join(map(lambda x: str(x), encrypted_msg))+plaintext

print('Длина текста', len(long_text))

long_text_compressed = zlib.compress(long_text.encode('utf-8'))

print('Длина текста_compressed', len(long_text_compressed))
...
msg = cipher.iv + cipher.encrypt(long_text_compressed)
print(msg)
